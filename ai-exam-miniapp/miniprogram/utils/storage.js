...
function modeLabel(mode) {
  ...
  if (normalized === 'upload_material') return '上传资料生成'
...
function initDefaults() {
  if (wx.getStorageSync(KEYS.POINTS) === '') {
    ...
    addPointTransaction({ type: 'new_user_bonus', amount: 3, remark: '新用户免费 3 点' })
  }
...
function addPoints(n, meta = {}) {
  ...
  addPointTransaction({
    type: meta.type || 'manual_adjustment',
    amount,
...
function bindInviter(inviteCode) {
  ...
  if (code === ownCode) return { success: false, code: 'SELF_INVITE_NOT_ALLOWED', message: '不能自己邀请自己。' }
...
function claimDailyTimelineShareReward(date = new Date()) {
  ...
  if (exists) {
    return {
      ...
      message: '你今天已经获得过分享奖励，明天再来吧。'
    }
  }
  addPoints(1, { type: 'daily_share_reward', remark: '每天分享到朋友圈奖励 1 点' })
  logs.unshift({
    id: `${Date.now()}_timeline_share`,
    rewardDate,
    rewardType: 'timeline_share',
    points: 1,
    createdAt: nowText()
  })
